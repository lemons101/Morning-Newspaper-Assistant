...
主链路是：`sources.yaml` 配置来源 -> `collect_raw.py` 采集候选 -> `enrich_content.py` 抓正文 -> 三个编辑回填关口完成标题粗筛、中文成稿、Top10 精排 -> `build_dashboard.py` 生成页面。邮箱提醒是独立侧链，只进入页面右侧提醒，不参与新闻 Top10 排序。
